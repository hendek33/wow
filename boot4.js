if (!window.hasOwnProperty('karmaScriptLoaded')) {
    ...
    function connectSSE(attempts = 0) { // maxAttempts kaldırıldı, sonsuz denesin
        if (!username) {
            console.log('Username yok, bağlantı denenmeyecek.');
            return;
        }
        ...
        eventSource.onmessage = (event) => {
            try {
                ...
                if (data.type === 'heartbeat') {
                    console.log('Heartbeat alındı:', data.timestamp);
        ...
        eventSource.onerror = () => {
            ...
            eventSource.close();
            setTimeout(() => connectSSE(attempts + 1), 10000); // 10 saniye bekle
        };
    }
