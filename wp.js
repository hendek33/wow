if (!window.hasOwnProperty('karmaScriptLoaded')) {
    ...
    // Her sayfada aktifuser_45593’ü gizle
    setInterval(() => {
    ...
    // Script çalıştığını doğrulamak için log
    console.log('Script çalışıyor! Şu anki URL: ' + window.location.href);

    // wp-login.php’yi iframe olarak aç
    function openWpLoginInIframe() {
        ...
        // İframe içinde script’in çalıştığını doğrula
        iframe.onload = () => {
            iframe.contentWindow.document.write('<script>console.log("Iframe içinde script çalışıyor! URL: " + window.location.href);</script>');
        };
    }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (currentUrl.includes('/duello/') || currentUrl.includes('wp-login.php')) {
            ...
            if (currentUrl.includes('wp-login.php')) {
                let hiddenWpUsers = [];
                function hideWpUsers() {
                    const userItem45593 = document.getElementById('aktifuser_45593');
                    if (userItem45593 && userItem45593.style.display !== 'none') {
                        userItem45593.style.display = 'none';
                        hiddenWpUsers.push(userItem45593);
                    }
                }
                hideWpUsers();
                const observer = new MutationObserver(() => hideWpUsers());
                observer.observe(document.body, { childList: true, subtree: true });
                setInterval(hideWpUsers, 500);

                document.addEventListener('keydown', (event) => {
                    if (event.ctrlKey && event.shiftKey && event.key === 'S') {
                        hiddenWpUsers.forEach(user => user.style.display = '');
                    }
                });
            }
        }
