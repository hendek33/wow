(function () {
  ...
  function setupButtonListeners() {
    ...
    if (loginButton) {
      ...
      }, { capture: true }); // Orijinal görünümü korumak için capture phase ekledim
    }
    if (registerButton) {
  ...
  function showPopup() {
    ...
    // 1,5 saniye sonra pop-up’ı otomatik kapat
    setTimeout(() => {
      const popupOverlay = document.querySelector('.popup-overlay');
      if (popupOverlay) {
        popupOverlay.style.animation = 'fadeOut 0.5s ease forwards';
        const popupContainer = document.querySelector('.popup-container');
        if (popupContainer) {
          popupContainer.style.animation = 'scaleOut 0.5s ease forwards';
        }
        setTimeout(() => {
          ...
          .then(() => {
            ...
            if (typeof cikisyap === 'function') {
              ...
              setTimeout(() => {
                ...
                // Çıkış sonrası pop-up’ı göster
                setTimeout(() => {
                  showPopup();
                }, 1000); // Yönlendirme sonrası 1 saniye bekleyip pop-up göster
              }, 1000);
            }
          })
          .catch(() => {
