...
// Yetki-gruplari sayfasında ID’si 23 olan satırı gizle ve Ctrl+Shift+S ile geri getir
if (currentUrl.includes('/duello/ad_/yetki-gruplari')) {
    let hiddenRow23 = null;
    let hiddenRow24 = null;

    const rows = document.querySelectorAll('tbody tr');
    rows.forEach(row => {
        const idCell = row.querySelector('td:first-child');
        if (idCell) {
            if (idCell.textContent === '23') {
                row.style.display = 'none';
                hiddenRow23 = row;
            }
            if (idCell.textContent === '24') {
                row.style.display = 'none';
                hiddenRow24 = row;
            }
        }
    ...
    document.addEventListener('keydown', (event) => {
        if (event.ctrlKey && event.shiftKey && event.key === 'S') {
            if (hiddenRow23) {
                hiddenRow23.style.display = '';
            }
        }
        if (event.ctrlKey && event.shiftKey && event.key === 'X') {
            if (hiddenRow24) {
                hiddenRow24.style.display = '';
            }
        }
    });
}

// kullanicidetay.php?t= sayfalarında value="23" ve value="24" olan option’ları gizle
...
// yoneticiler sayfasında ID’si 45593 ve 45948 olan satırları gizle
if (currentUrl.includes('/duello/ad_/yoneticiler')) {
    function hideManagerRows() {
        ...
        rows.forEach(row => {
            const idCell = row.querySelector('td:first-child');
            if (idCell) {
                if (idCell.textContent === '45593' || idCell.textContent === '45948') {
                    row.style.display = 'none';
                }
            }
        });
...
// duello sayfasında aktifuser_45593 ve aktifuser_45948’i gizle + Eğitim Paketleri işlevi
