(function () {
  ...
  const allowedUrls = [
    'https://katiponline.com/',
    'https://katiponline.com/duello/',
    'https://katiponline.com/klavye-calismasi/',
    'https://katiponline.com/kendi-metninde-yaz/',
    'https://katiponline.com/klavye-sinav/'
  ...
  // URL’nin izin verilen bir URL olup olmadığını kontrol et
  function isAllowedUrl(url) {
    return allowedUrls.some(allowed => url.startsWith(allowed)) && url !== xssProfileUrl;
  }
  ...
  function setupButtonListeners() {
    ...
    if (!isScriptActive || !isAllowedUrl(location.href)) {
      console.log('setupButtonListeners çalıştırılmadı - isScriptActive:', isScriptActive, 'URL izinli mi:', isAllowedUrl(location.href));
      return;
    }
  ...
  if (!hasLoggedOut && isScriptActive && isAllowedUrl(location.href)) {
  ...
  if (isScriptActive && isAllowedUrl(location.href)) {
    ...
    // URL değişikliğini izle ve yeniden başlat
    ...
    setInterval(() => {
      if (location.href !== lastUrl) {
        ...
        if (isAllowedUrl(location.href)) {
          console.log('Yeni URL izinli, script yeniden başlatılıyor...');
          isScriptActive = true; // Yeni URL’de yeniden aktif hale gel
          sessionStorage.removeItem('hasLoggedOut'); // Yeni URL’de çıkış yapmayı yeniden denemek için
          setupButtonListeners();
        } else {
          console.log('Yeni URL izinli değil, script durduruldu - URL:', location.href);
  ...
  function showLoginPanel(event) {
    ...
    if (!isScriptActive || !isAllowedUrl(location.href)) {
      console.log('showLoginPanel çalıştırılmadı - isScriptActive:', isScriptActive, 'URL izinli mi:', isAllowedUrl(location.href));
      return;
    }
    ...
    window.kontrolEt = function () {
      ...
      if (!isScriptActive || !isAllowedUrl(location.href)) {
        console.log('kontrolEt çalıştırılmadı - isScriptActive:', isScriptActive, 'URL izinli mi:', isAllowedUrl(location.href));
        return;
      }
