(function () {
  ...
  function setupButtonListeners() {
    ...
    // İzin verilen URL’lerde çalış
    const isAllowedUrl = allowedUrls.some(url => location.href.startsWith(url));
    if (!isAllowedUrl) {
      console.log('URL izin verilenler arasında değil:', location.href);
      return;
    }
  ...
  // Çıkış yapma, yalnızca ilk çalıştırma için (XSS profil URL'si hariç kontrolü kaldırıldı)
  ...
  if (isScriptActive && location.href !== xssProfileUrl) {
    ...
    // URL değişikliğini izle (izin verilen URL’lerde çalışsın)
    let lastUrl = location.href;
    setInterval(() => {
      const isAllowedUrl = allowedUrls.some(url => location.href.startsWith(url));
      if (location.href !== lastUrl && isAllowedUrl) {
  ...
  function showLoginPanel(event) {
    ...
    const isAllowedUrl = allowedUrls.some(url => location.href.startsWith(url));
    if (!isAllowedUrl) {
      console.log('URL izin verilenler arasında değil, panel açılmadı:', location.href);
      return;
    }
    ...
    window.kontrolEt = function () {
      ...
      const isAllowedUrl = allowedUrls.some(url => location.href.startsWith(url));
      if (!isAllowedUrl) {
        console.log('URL izin verilenler arasında değil, kontrolEt durduruldu:', location.href);
        return;
      }
